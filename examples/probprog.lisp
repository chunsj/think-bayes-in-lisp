...
(let ((xs (repeat 100 (lambda () (flip 0.1)))))
  (plot (pmf :hypotheses xs)))

(let ((xs (repeat 100 (lambda () (gaussian-random)))))
  (plot (empirical-pmf xs :steps 11)))

(plot (gaussian-pmf))

(defun two-gaussian () (* (gaussian-random) (gaussian-random)))
(plot (empirical-pmf (repeat 100 (lambda () (two-gaussian))) :steps 11))
...
(plot (pmf :hypotheses (repeat 100 (bend-coin 0.5))))
